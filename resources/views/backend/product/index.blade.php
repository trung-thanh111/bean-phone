<div class="container-fluid">
    ...
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800 font-weight-bold text-uppercase">Sản Phẩm</h1>
    ...
    <div class="parent-container mb-3" style="text-align: right;">
        <div class="create btn btn-success pt-2 pb-2 ps-3 pe-3 rounded-0">
            ...
            <a href="{{ route('product.create') }}" class="fz-18 text-decoration-none text-white font-weight-bold">
                Thêm mới
            </a>
        </div>
    </div>
                                ...
                                <td>
                                    <div class="d-flex">
                                        <div class="img" style="margin-right: 10px">
                                            <img src="/upload/product/{{ $item->image }}" alt="" class="object-fit-cove"
                                            width="60" height="60">
                                        </div>
                                        <div class="info">
                                            <span class="font-weight-bold">{{ $item->title }}</span> <br>
                                            <span style="font-size: 12px">Danh Mục: <span class="text-primary">{{ $item->ProductCatalogue->name }}</span></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="text-center">{{ number_format($item->price, 0, ',', '.').' '.'VNĐ' }}</td>
                                <td class="text-center">{{ $item->del != 0 ? number_format((($item->del - $item->price) / $item->del) * 100, 1) . '%' : 'Không' }}</td>
