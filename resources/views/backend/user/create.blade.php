<div class="wrapper">
    <div class="title-create pr-4 pl-4 pb-4">
        <h4 class="font-weight-bold text-uppercase">Thêm mới thành viên</h4>
        <p>Bạn hãy chắc chắn thông tin mà bạn đang nhập đúng với những gì đang có!</p>
    </div>
    <form action="{{ route('user.store') }}" method="POST" class="p-2" enctype="multipart/form-data">
        @csrf
        ...
        <div class="row pr-3 pl-3 pt-3">
            <div class="form-group col-md-6">
                ...
                <input type="text" name="email" class="form-control rounded-0 pt-1 pb-1"
                    value="{{ old('email') }}">
                @if ($errors->has('email'))
                    <span class="text-danger fz-14">{{ $errors->first('email') }}</span>
                @endif
            </div>
            <div class="form-group col-md-6">
                <label class="font-weight-bold">Mật khẩu:</label>
                <input type="text" name="password" class="form-control rounded-0 pt-1 pb-1"
                    value="{{ old('password') }}">
                @if ($errors->has('password'))
                    <span class="text-danger fz-14">{{ $errors->first('password') }}</span>
                @endif
            </div>
        </div>
        ...
        <div class="row pr-3 pl-3 pt-3">
            ...
            <div class="form-group col-md-4">
                ...
                <input type="date" name="birthday"  class="form-control rounded-0" value="{{ old('birthday') }}">
            </div>
            <div class="form-group col-md-4">
                ...
                <select class="form-control rounded-0 setupSelect2" name="user_catalogue_id">
                    ...
                    <option value="{{ $item->id }}" {{ old('user_catalogue_id') == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                    @endforeach
                </select>
