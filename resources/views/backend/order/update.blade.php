<div class="wrapper">
    <div class="title-create p-4 pt-0">
        <h4 class="font-weight-bold text-uppercase">Cập nhật sản phẩm</h4>
        <p>Bạn hãy chắc chắn thông tin mà bạn đang nhập đúng với những gì đang có!</p>
    </div>
    <form action="{{ route('product.edit', $product->id) }}" method="POST" class="p-2" enctype="multipart/form-data">
        @csrf
        <div class="row p-3">
        ...
        <div class="form-group col-12">
            ...
            <textarea type="text" name="short_desc" class=" ckeditor form-control rounded-0 pt-1 ps-2 pe-2 pb-1" rows="3"
                placeholder="Nhập mô tả">{{ old('short_desc', $product->short_desc) }}</textarea>
        ...
        <div class="row p-3">
            <div class="form-group col-md-3">
                ...
                <select class="form-control rounded-0 setupSelect2" name="accessory">
                    <option value="0" {{ old('accessory', $product->accessory) == 0 ? 'selected' : '' }}>Sản
                        phẩm đặc trưng</option>
                    <option value="1" {{ old('accessory', $product->accessory) == 1 ? 'selected' : '' }}>Phụ
                        kiện</option>
                </select>
                ...
                <select class="form-control rounded-0 setupSelect2" name="product_catalogue_id">
                    @foreach ($categories as $key => $item)
                        <option value="{{ $item->id }}"
                            {{ old('product_catalogue_id', $product->product_catalogue_id) == $item->id ? 'selected' : '' }}>
                            {{ $item->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-3">
                ...
                <select class="form-control rounded-0 setupSelect2" name="publish">
                    <option value="1" {{ old('publish', $product->publish) == 1 ? 'selected' : '' }}>Hiển thị
                    </option>
                    <option value="0" {{ old('publish', $product->publish) == 0 ? 'selected' : '' }}>Ẩn
                    </option>
                </select>
        ...
        <img src="/upload/product/{{ $product->image }}" alt="" width="160" class="object-fit-cover">
