<div class="wrapper">
    ...
    <form action="{{ route('product.catalogue.edit', ['id' => $productCatalogue->id]) }}" method="POST" class="p-2" enctype="multipart/form-data">
        @csrf
        <div class="row p-3">
        ...
        <div class="form-group col-12">
            ...
            <textarea type="text" name="description" class="form-control rounded-0 pt-1 ps-2 pe-2 pb-1" rows="10"
                placeholder="Nhập mô tả">{{ old('description', $productCatalogue->description) }}</textarea>
        </div>
        <div class="row p-3">
            <div class="form-group col-md-6">
                ...
                <select class="form-control rounded-0 setupSelect2" name="parent_id">
                    ...
                    @foreach ($categories as $key => $item)
                        <option value="{{ $item->parent_id }}" {{ old('parent_id', $productCatalogue->parent_id) == $item->parent_id ? 'selected' : '' }}>
                            {{ $item->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control rounded-0 setupSelect2" name="publish">
                    ...
                    <option value="1" {{ old('publish', $productCatalogue->publish) == 1 ? 'selected' : '' }}>Hiển thị</option>
                    <option value="0" {{ old('publish', $productCatalogue->publish) == 0 ? 'selected' : '' }}>Ẩn</option>
                </select>
            </div>
        </div>
        <div class="form-group col-md-6">
            <label for="inputState" class="font-weight-bold">Hình ảnh:</label> <br>
            <input type="file" name="image" value="{{ old('image', $productCatalogue->image) }}">
        </div>
        <img src="/product/upload/{{ $productCatalogue->image }}" alt="" class="object-fit-cover ml-3" width="120" height="120">
