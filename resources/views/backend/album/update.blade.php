<div class="wrapper">
    <div class="title-create pr-4 pl-4 pb-4">
        <h4 class="font-weight-bold text-uppercase">Cập nhật album</h4>
        <p>Bạn hãy chắc chắn thông tin mà bạn đang nhập đúng với những gì đang có!</p>
    </div>
    <form action="{{ route('album.edit', $album->id) }}" method="POST" class="p-2" enctype="multipart/form-data">
        @csrf
        <div class="form-group col-12">
            <label class="font-weight-bold">Mô tả:</label>
            <textarea type="text" name="description" class="form-control rounded-0 pt-1 ps-2 pe-2 pb-1" rows="10"
                placeholder="Nhập mô tả">{{ old('description', $album->description) }}</textarea>
        </div>
        <div class="form-group col-md-12">
            <label class="font-weight-bold">Albums:</label> <br>
            @php
                $albumSubs = json_decode(str_replace("'", '"', $album->albums), true);
            @endphp
            @foreach ($albumSubs as $item)
                <img src="/upload/product/{{ $item }}" alt="" width="200" class="mb-3">
            @endforeach
        ...
        <div class="row p-3">
                ...
                <select name="product_id" class="form-control rounded-0 setupSelect2">
                    ...
                    @foreach ($products as $key => $item)
                        <option value="{{ $item->id }}"
                            {{ isset($album) && $item->id == $album->product_id ? 'selected' : '' }}>
                            {{ $item->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control rounded-0 setupSelect2" name="publish">
                    ...
                    <option value="1" {{ old('publish',isset($album) && $album->publish) == 1 ? 'selected' : '' }}>Hiển thị
                    </option>
                    <option value="0" {{ old('publish', isset($album) && $album->publish) == 0 ? 'selected' : '' }}>Ẩn</option>
                </select>
            </div>
        </div>
