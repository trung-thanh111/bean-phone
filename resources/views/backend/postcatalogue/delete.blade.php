<div class="wrapper p-2">
    <div class="title-create pl-4 pb-4">
        <h4 class="font-weight-bold text-uppercase mb-3">xóa nhóm sản phẩm</h4>
    ...
    <div class="row pr-4 pl-4 mt-3">
        <div class="col-md-6">
            ...
            <div class="fz-14">
                <p class="mb-1">Bạn hãy chắc chắn muốn xóa thông tin nhóm bài viết <strong class="text-danger">id =
                        {{ $postCatalogue->id }}</strong>.</p>
                <p>Sau khi xóa thông tin sản phẩm sẽ không thể phục hồi!</p>
            </div>
        </div>
        <div class="col-md-6">
            <form action="{{ route('post.catalogue.destroy', ['id' => $postCatalogue->id]) }}" method="POST">
                @csrf
                @method('DELETE')
                <div class="mb-4">
                    <label class="form-label">Tên nhóm:</label>
                    <input type="text" class="form-control rounded-0" value="{{ $postCatalogue->name }}" readonly>
                </div>
                <div class="d-flex justify-content-end">
                    <div class="submit w-50 d-flex ">
                        <a href="{{ route('admin.post.catalogues') }}"
