...
<div class="container mb-5  ">
    <div class="row mt-4">
        @php
            $albumProducts = json_decode(str_replace("'", '"', $bannerProducts->albums), true);
        @endphp

        @foreach ($albumProducts as $key => $album)
            <div class="col-4">
                <img src="/upload/banner/{{ $album }}" class="rounded-2 me-2 w-100 object-fit-cover" width="416" height="172" alt="banner sản phẩm">
            </div>
        @endforeach
    </div>
    <div class="row">
    ...
    <div class="row mt-4">
                    ...
                    @foreach ($allCategories as $key => $item)
                        <div class="form-check mb-2">
                            <input class="form-check-input category-checkbox" type="checkbox"
                                value="{{ $item->id }}" id="categoryCheckbox{{ $key }}">
                            <label class="form-check-label" for="categoryCheckbox{{ $key }}">
                                {{ $item->name }}
                            </label>
                        </div>
                    @endforeach
        ...
        <div class="col-9">
            <div class="d-flex justify-content-between">
                <h1 style="font-size: 18px; font-weight: bold;" class="mb-2 mt-2 rounded-2 bg-light pt-1 pb-1 ps-1">
                    TẤT CẢ SẢN PHẨM</h1>
                <select id="sortSelect" class="form-select bg-body-secondary text-dark p-0 ps-3 rounded-0 fz-14"
                    style="width: 180px; height: 30px;">
                    <option value="default" selected><i class="fa-solid fa-filter"></i> Sắp Xếp: Mặc Định</option>
                    <option value="az">A - Z</option>
                    <option value="za">Z - A</option>
                    <option value="lowtohigh">Giá Trị Thấp Đến Cao</option>
                    <option value="hightolow">Giá Trị Cao Đến Thấp</option>
                </select>
            </div>
                    ...
                    <div class="col-md-3 pt-2 ps-3 mb-4">
                        <div class="card shadow">
                            ...
                            <img src="fontend/img/{{ $item->image }}"
                                class="card-img-top scale-hover object-fit-cover" alt="..." width="231"
                                height="231">
                            <span
